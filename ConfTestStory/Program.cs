    ...
    class Configuration<T> where T : new()
    {
        ...
        public void ApplyArguments(string[] args)
        {
            foreach(string arg in args)
            {
                ...
                {
                    var argValArr = new string[argData.Length - 1];
                    Array.Copy(argData, 1, argValArr, 0, argData.Length - 1);
                    argValue = string.Join("=", argValArr);
                }

                var pathData = argPath.Split('.');
                string propName = pathData[0];
                object objProp = config;
                var propInfo = objProp.GetType().GetProperty(propName);

                for (var i = 1; i < pathData.Length - 1; i++)
                {
                    propName = pathData[i];

                    propInfo = objProp.GetType().GetProperty(propName);
                ...
                if (propInfo != null)
                {
                    objProp.GetType().GetProperty(propName).SetValue(objProp, argValue);
                }
            }
        }
